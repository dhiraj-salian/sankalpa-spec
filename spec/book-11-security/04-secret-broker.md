...
*Nature: **Normative**. · Reflects: SECURITY.md, RFC-0001, RFC-0005 (rotation generation; Execution-scoped rotation); realizes principle P7 (and P8, P9, P10). Companion to Book 06 §Ch06 (materialization), Book 02 §Ch06 §4 (secret references).*
...
- **Rotation** is first-class: a secret can be rotated (new value, same reference) so that references remain stable while values change (the reverse of the reference model's other benefit). Rotation is audited; **consumers are unaffected at Execution granularity** — a `SecretRef` resolves to a stable value for an Execution's lifetime (Book 06 §Ch06 §2.1), so a rotation landing mid-flight applies to the *next* Execution that materializes the reference, never to one already underway. Stated unqualified, "consumers are unaffected because they hold references, not values" would be false for a consumer that materializes the same reference twice: without the Execution boundary, a mid-run rotation would silently change a resolved binding (Book 06 §Ch03 §1).
- **Rotation generation.** Each `Secret` carries a monotonic, opaque **`rotationGeneration`**, incremented on every rotation or revocation. It is a version counter — an integer/opaque token, **not** a hash of the value — so it carries no value material and leaks nothing under P7. It is returned with each materialization (Book 06 §Ch06 §2), recorded in the materialization audit (§6) and on the `Execution` (by reference), and it is what lets **re-execution replay** detect that a secret changed since the recorded run and fail closed rather than silently perform a secret-dependent effect under a different credential (Book 06 §Ch03 §1). Policy (Ch 06) MAY read it — e.g. requiring "materialize only the latest generation" for high-consequence classes after a suspected compromise. Because rotation is *not* revocation, a generation change is **not** a drift signal for determinized Capabilities (Book 10 §Ch06 §5): it changes an effect's credential, not a reasoning node's input→output relation, and treating it as drift would retire healthy Capabilities on routine rotation — pressuring against a practice this section mandates.
