...
*Nature: **Normative** (defines terms the whole spec depends on). · Reflects: RFC-0001; realizes P1, P13. Companion to Book 04 §Ch02, Book 05 §Ch06, Book 10 §Ch06.*
...
**Determinism in Sankalpa means: given the same Low IR, the same resolved inputs, and the same resolved bindings, execution produces the same observable effects and outputs — reproducibly** (Book 04 §Ch02 IR-P1, Book 06 §Ch03 §1).
...
- **Determinism is about the machine, not the model.** The spec assumes *no* model determinism (Book 08 §Ch05 §5). A model call is a `Reasoning` node whose actual output is captured (Book 10); the deterministic guarantees hold around it regardless of how the model behaves.
...
1. Determinism means reproducible observable behavior given the same Low IR, inputs, and bindings; it is relative to *declared* inputs/effects and does not assume an unchanging world or a deterministic model.
