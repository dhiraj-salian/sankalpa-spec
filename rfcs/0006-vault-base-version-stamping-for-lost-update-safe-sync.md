...
> **Accepted 2026-07-16.** FCP (accept disposition) was called and concluded the same day by the Knowledge Domain Lead. For the solo-maintainer repo the 10-working-day window was shortened and the ≥2-Reviewer gate ([process §7](../process/rfc-process.md)) waived — both recorded here for auditability, not pretended. No blocking objections; all design questions resolved (see *Resolved questions*), and the review pass fixed the defects it found. Per [process §8](../process/rfc-process.md) this RFC becomes **normative only on reflection into `spec/`**; status advances to **Final** once the Documentation Changes (§12) land in Book 09 §03/§05 and the Glossary.

> **Final 2026-07-16.** §12 reflected into `spec/` (Book 09 §03 front-matter schema, §05 §4/§7 conflict resolution, §06 retrieval) and the Glossary in this change; the RFC is now normative. See CHANGELOG.
