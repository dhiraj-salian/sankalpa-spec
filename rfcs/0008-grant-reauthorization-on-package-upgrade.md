...
- Should the authoritative "sensitive capability class" set be defined here, in [Book 11 §Ch07](../spec/book-11-security/07-approval-engine.md) (approval triggers), or in policy — and is it workspace-extensible?
- For a same-key upgrade that *narrows* capabilities, should the now-unused grants be auto-revoked (least-privilege drift correction) or left until uninstall?
- Should a downgrade (rollback to older-than-current, or install of a lower version) that re-introduces a *wider* capability set than the current one trigger §4.2 broadening even though the target is "older"?
