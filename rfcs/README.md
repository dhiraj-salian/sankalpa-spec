...
>
> RFC-0005–0011 are the **second Phase 2 hardening batch**, **Final 2026-07-16** — accepted (FCP accept disposition, called and concluded the same day) and reflected into `spec/`, the Glossary, and the CHANGELOG across seven changes. Solo-maintainer repo: the 10-working-day FCP window was shortened and the ≥2-Reviewer gate waived, both recorded in each RFC header for auditability. All 21 open design questions were resolved before proposing, and a review pass fixed two defects it found (0005 conflated rotation with revocation, which would have let a memoized value shield a revoked grant against Book 11 §03 §5's "even past any caching"; 0007 claimed deadline-less work could be held indefinitely, contradicting its own shed budget and Book 03 §13 §2). Each pins one concrete gap in a normative guarantee: **0005** — secret values are unrecordable (P7) resolved bindings, so intra-execution stability and the secret carve-out were unspecified. **0006** (Knowledge) — vault↔graph optimistic concurrency had no baseline for offline direct edits. **0007** (Kernel) — scheduling fairness/deadline/shed promises were unbacked and `Pending` could stall silently. **0008** (Packages) — grants were not re-authorized on upgrade (upgrade-laundering). **0009** (Interfaces) — messaging-channel sender identity was mapped, not authenticated. **0010** (Security/Observability) — the "hard" log secret-freedom guarantee rested on "nothing secret on the path", false for the untrusted runtime holding the only plaintext secret and emitting the Events the ledger is built from. **0011** (Compiler/IR) — IR-P10 cited a "semantics-preservation check" defined nowhere; the checks that existed never compared a pass's output to its input. Reflection touched Books 01–06 and 09–14 plus the Glossary and CHANGELOG. **0011 §6 is breaking** (the pass contract) and lands per its staged migration.
