...
## 7. Open questions
- **Workers removes non-determinism from the API; we remove it from the representation. Is there a third move available for the planner?** Workers' timing defenses assume an adversary *inside* the sandbox and answer by making the attack unexpressible. Our planner faces untrusted *input* (prompt injection, Book 11 §Ch01 §3) and we answer by confinement: the planner holds capability *descriptors* to reason with, not execution authority (Book 11 §Ch10 §3, Book 08 §Ch04), and its output is verified before it means anything. That is sound, but it is containment, not unexpressibility — a prompt-injected planner still emits a plan; we merely check it. Whether the Workers move has an analog here, or whether verification is genuinely the only tool against an adversarial front-end, is the interesting question and Book 08 §Ch05 does not ask it.

*Checked and answered, contrary to an earlier draft of this study: runtime selection on declared effects (Book 06 §Ch04 §3.1 — a runtime is a candidate **only if** `supports(lowModule)` holds for every instruction, effect, and `ExecPolicy`; the Runtime Manager "MUST NOT select it and hope", and if no runtime qualifies, selection fails with a typed error rather than executing on an unfaithful engine — a CPU-limited runtime is excluded statically from the module's declared needs, exactly as declared effects intend). Minimum isolation strength (Book 11 §Ch10 §1 — the boundary's *properties* are normative and the mechanism is deliberately open; the undefined term is "plugin class", see the [Docker study](docker.md)).*
