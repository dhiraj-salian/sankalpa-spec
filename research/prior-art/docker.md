...
## 7. Open questions
- **The Package payload format is unspecified.** Book 12 specifies the manifest (§Ch02), resolution (§Ch03), signing and provenance (§Ch04), install (§Ch05), and the marketplace (§Ch06) — everything *about* the artifact, and nothing about what the artifact *is*. No chapter mentions a container image, an archive format, OCI, or any encoding. This is the single largest unfilled space in Book 12, and it is not obviously an oversight: leaving it open preserves the choice, and adopting OCI would be an enormous free ride on existing registry infrastructure bought with a permanent constraint on the manifest. Either way it should be a decision on the record rather than an absence.
- **Plugin classes are named but not defined.** Book 11 §Ch10 §1 does state a normative minimum — a list of properties the boundary MUST provide — and deliberately leaves the mechanism (process, container, VM, WASM) an implementation choice "matched to the threat and the plugin class". But *plugin class* is never enumerated. Whether a shared-kernel container clears the bar for a Package from an unknown marketplace publisher, versus a first-party runtime, is exactly the judgment the phrase defers and nothing resolves.
- **What is our `docker build`?** Docker's ecosystem exists because producing the artifact was trivial. Book 12 §Ch05's install path is well-specified from the *consumer* side; the author's path — write a Capability, declare effects, pass conformance, sign, publish — is specified in pieces across §Ch02/§Ch04/§Ch06 and nowhere end-to-end. Ecosystems form on the authoring gradient.
