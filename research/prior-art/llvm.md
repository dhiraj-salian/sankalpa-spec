...
## 7. Open questions
- **Effect-graph conservation is a structural check sold against a semantic threat.** Book 05 §Ch02 §4 is admirably honest that it catches unintended collateral change and decides nothing at the value level, with translation validation (§4.3) the opt-in that goes further — required only for a pass running after an `Approval` is lowered in. That leaves the interesting middle: a third-party pass that conserves the effect graph exactly and still computes a different value. RFC-0011's motivating attack (retargeting an approved transfer's payee) is caught because target identity is part of conservation; an attack on a *value* that is not an effect target is not. Whether that residue is worth closing — by widening the `validated: true` requirement past the approval trigger — is a live cost/benefit question, not a settled one.

*Checked and answered, contrary to an earlier draft of this study: intra-stage pass ordering (Book 05 §Ch02 §5 — order is deterministic, versioned, part of the compilation cache key, topologically scheduled from `requires`/`invalidates`, cycles rejected, and pinned per compilation on the `Compilation` Resource; the phase-ordering free-for-all is structurally foreclosed). IR migration (Book 04 §Ch09 §4 — version-tagged modules, multi-version tooling, lowering-compatibility shims, and migration-not-mutation; our promise is indeed stronger than LLVM's and the mechanism is specified).*
