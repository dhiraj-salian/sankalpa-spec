...
- **A single hash function baked into identity.** Git's SHA-1 migration is the warning, and Book 04 §Ch09 §5 already heeds it: the digest algorithm is versioned in IR metadata, a new algorithm is a MINOR addition, existing modules keep their identity, the store may carry dual identities through a transition, and retiring an old algorithm is MAJOR. This is the agility Git had to retrofit, specified up front.
...
## 7. Open questions
- **Canonicalization is specified by example at two load-bearing points.** Book 04 §Ch07 §2 is otherwise exact — six MUSTs, "deterministic and total" — but §2.1 says ids are assigned canonically "*e.g.* by a deterministic topological numbering" and §2.2 that sets are emitted in a defined canonical order "*e.g.* by structural hash". An `e.g.` is not a specification. Two conforming implementations that pick different topological numberings or different set orders produce different bytes, hence different hashes, hence different identities for the same module — and every property built on identity (caching, replay, determinization's "we have seen this exact work before", signing) silently degrades to per-implementation. Either the algorithms are fixed normatively or the conformance suite must pin them with cross-implementation golden files. Compare Git, where the object format is exact to the byte precisely because identity depends on it.
- **Erasure has no mechanism.** Retention is specified (Book 02 §Ch04 §5's per-kind policies, Book 11 §Ch09 §6's audit retention), and that answers *time-based reclamation*. It does not answer a **targeted deletion obligation** — erase this subject's data — landing on a trail that Book 11 §Ch09 §4 requires to be tamper-evident and append-only, where "corrections are new records, never edits". Git has no answer either (the `filter-branch`-and-rotate incident is the canonical one), but Git does not promise auditability as evidence. Neither crypto-shredding nor tombstones appear anywhere in `spec/`.

*Checked and answered, contrary to an earlier draft of this study: digest agility (Book 04 §Ch09 §5, §6 above).*
