...
## 7. Open questions
- If `status` is linearizable and Watch is level-based, is CQRS actually load-bearing here — or are we citing a pattern whose real work is being done by reconciliation ([Kubernetes study](../prior-art/kubernetes.md))? Book 01 §Ch06 does not list CQRS among the influences, which may be the honest answer.
- Book 14's projections (metrics, audit) have retention and tenancy rules (Book 02 §Ch08 §7) that the Event stream may not share. Is a projection genuinely rebuildable after the source events age out — and if not, the projection *is* a record, with all the obligations that implies?
- Does any read path in the system serve from an async projection today? If Book 14 §Ch08's operability surfaces do, their staleness bounds should be stated where users meet them.
