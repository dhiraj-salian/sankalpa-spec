...
## 7. Open questions
- **Renaming a term has no specified path, and the spec is now large enough for that to bind.** The [Glossary](../../GLOSSARY.md) is a living document and [`rfc-process.md`](../../process/rfc-process.md) exempts "clarifications that preserve meaning" from needing an RFC — but a *rename* does not preserve the text, and the documentation gate ([`review-gates.md`](../../process/review-gates.md)) requires `spec/`, Glossary, and diagrams to move together. Across 16 Books and 11 Final RFCs, the reflection cost of one rename is now substantial and nobody is designated to bear it. Since §6 concedes our language is stipulated rather than elicited — and therefore *expected* to be wrong somewhere — this is the mechanism the project will need first and specified last.
- **Is Knowledge really the same concept as an Execution?** P2 says everything is a Resource, and Book 09 §Ch03's vault is an Obsidian-shaped, human-edited, file-backed thing that RFC-0006 had to give three-way merge and base-version stamping — machinery no other Resource needs, because no other Resource is edited outside the Kernel. That is either a Resource with unusual sync requirements or a second model wearing the word, and DDD's bounded-context lens is what makes the question askable.
- **Is there a bounded context we are refusing to admit?** Planning (Book 08) and Execution (Book 06) reason about "a plan" differently enough that the High/Low IR split (Book 04 §Ch03–04) arguably *is* that seam — admitted in the representation rather than in the language. If so, the honest reading is that we do have internal contexts, and the IR is our context map.
