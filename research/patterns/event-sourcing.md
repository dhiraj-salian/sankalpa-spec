...
## 7. Open questions
- **Erasure has no mechanism, and this is the pattern's known collision arriving on schedule.** Retention is specified — Book 02 §Ch04 §5's per-kind policies with owner-based cascading GC, Book 11 §Ch09 §6's audit retention — and that is *time-based reclamation*, which is not erasure. A targeted deletion obligation ("remove this subject") lands on a trail Book 11 §Ch09 §4 requires to be **tamper-evident**, where "corrections are new records, never edits". Those two requirements cannot both hold for the same datum, and `spec/` names neither crypto-shredding nor tombstones nor any escape. The [Git study](../prior-art/git.md) reaches this from the other side. Of everything this pattern predicts will hurt, this is the one that is genuinely unaddressed.
- **Can ARM and the log disagree, and what would notice?** §6 above concedes we are event-*driven* with an authoritative state store rather than purely event-sourced — a configuration a purist cannot have this bug in, and we can. Book 14 §Ch01 §5 forbids dual-write at the model level, which is the right prophylactic; nothing specifies a *detector*. A reconciliation between the store and the stream is the kind of thing one wants before an auditor asks, not after.

*Checked and answered, contrary to an earlier draft of this study:*
- *The retention horizon's effect on learning is smaller than feared — Book 02 §Ch04 §5 states that "often the derived Experience outlives the Execution Resource", so determinization's evidence base (Book 10 §Ch06 §3) is not bounded by Resource retention. Audit records are likewise "often longer than the Resources they describe" (Book 11 §Ch09 §6).*
- *Version-tolerant readers are required, not merely hoped for, at least for IR: Book 04 §Ch09 §3 mandates that a consumer meeting a MINOR it does not know **MUST reject** what it does not understand rather than guess, and MUST NOT silently ignore an unknown node or effect "because that could drop meaning or an effect Policy needed to see" — deny-by-default (IR-P3) extended to unknown constructs. Book 14 §Ch01 §4's additive-only rule is the Event-stream analog.*
