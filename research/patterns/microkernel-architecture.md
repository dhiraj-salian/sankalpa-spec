...
## 7. Open questions
- Eighteen Managers is a lot of trusted code. Is there a stated *budget* or review gate for core growth, or only a litmus test that each addition individually passes? The distributed monolith is built entirely out of individually reasonable decisions.
- The Kernel API is a single front door and therefore a single bottleneck. Book 03 §Ch13 covers degradation, but is there a specified path to a *horizontally scaled* Kernel, and does P4 survive it intact?
- seL4 shows minimality enables proof. Have we given up formal assurance of the core, or is some invariant subset (the security invariants in Book 11 §Ch11) a candidate for mechanized verification?
