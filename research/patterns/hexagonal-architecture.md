...
## 7. Open questions
- **Two ports have conformance suites; the rest have promises.** Books 06 §Ch07 and 08 §Ch07 give runtimes and planners executable contracts, and Book 11 §Ch11's SI-6 leans on them ("self-claims are conformance-checked, never trusted"). Book 13 contains no conformance suite for channel adapters, and providers (Book 03 §Ch04) have none either — yet a channel adapter is an *ingress* trust boundary (Book 13 §Ch02 §3, Book 11 §Ch02 §B1) that RFC-0009 had to harden precisely because the Kernel was taking an untrusted adapter's word about identity. §6 above argues a port without a conformance suite is a promise rather than a port; by that standard we currently have two ports and several promises.
- **Is a plan using a `Custom` effect still portable?** IR-P7 says the IR is runtime-agnostic and Book 06 §Ch04 §3.1 enforces it at selection — a runtime that cannot honor every effect is not a candidate, and if none qualifies, selection fails loudly. So the *mechanism* is honest: an unportable plan fails closed rather than silently binding to one engine. But the *ecosystem* consequence is the one every portable abstraction with an escape hatch has met — if the good runtime's distinctive capability is reachable only through a `Custom` effect, plans that use it run on exactly one runtime, and we have a portable IR with unportable programs. Nothing forbids that; Book 06 §Ch04 merely makes it visible.
- **Would we notice a leaked concept?** The port-vocabulary rule (§5) is easy to state and hard to hold, and Book 06 §Ch02's runtime interface will in practice be co-designed with its first implementer. That is how every "abstract" interface acquires one engine's assumptions.
